const StartFunc = ({ inFromFetch, InTechnicianData }) => {
    ...
    jFLocalToInputEngineerDatalistId({ InternalStatus: inFromFetch.InternalStatus })
    jFLocalToInputEngineerRemarksId({ EngineerRemarks: inFromFetch.EngineerRemarks })
};
...
let jFLocalToInputEngineerDatalistId = ({ InternalStatus }) => {
    let jVarLocalHtmlId = 'EngineerId';
    let jVarLocalEngineerDatalistId = document.getElementById(jVarLocalHtmlId);

    if (jVarLocalEngineerDatalistId === null === false) {
        jVarLocalEngineerDatalistId.value = InternalStatus;
    };
};

let jFLocalToInputEngineerRemarksId = ({EngineerRemarks}) => {
    let jVarLocalHtmlId = 'EngineerRemarksId';
   let jVarLocalEngineerRemarksId = document.getElementById(jVarLocalHtmlId);
   
   if (jVarLocalEngineerRemarksId === null === false) {
      jVarLocalEngineerRemarksId.value = EngineerRemarks;
   };
};
